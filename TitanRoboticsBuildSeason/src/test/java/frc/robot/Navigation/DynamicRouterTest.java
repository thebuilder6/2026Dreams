...
import frc.robot.Navigation.DynamicRouter.AvoidanceAlgorithm;
...
public class DynamicRouterTest {
    ...
    @Test
    public void testPotentialFieldsHeadOnAvoidance() {
        DynamicRouter.setAlgorithm(AvoidanceAlgorithm.POTENTIAL_FIELDS);
        ...
        // APF should choose a lateral deviation (non-zero vy) or reduced speed to avoid head-on collision
        ...
        assertTrue(hasLateralEvasion || hasReducedSpeed,
                "APF should either evade laterally or brake to prevent collision");
    }

    @Test
    public void testPotentialFieldsCorridorAvoidance() {
        DynamicRouter.setAlgorithm(AvoidanceAlgorithm.POTENTIAL_FIELDS);
        ...
        // Corridor is blocked at Y=4.0, so APF should deflect around (non-zero velocity)
        assertTrue(Math.hypot(avoidanceSpeeds.vxMetersPerSecond, avoidanceSpeeds.vyMetersPerSecond) > 0.1,
                "APF should produce valid velocity vector along open corridor");
    }
